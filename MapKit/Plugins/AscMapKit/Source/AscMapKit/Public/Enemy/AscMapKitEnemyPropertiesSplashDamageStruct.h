...
struct ASCMAPKIT_API FAscMapKitEnemyPropertiesSplashDamageStruct
{
    ...
    // If enabled, splash damage to players/other enemies will occur when the enemy dies/explodes.
    UPROPERTY(EditAnywhere, BlueprintReadWrite, SaveGame)
    bool Enable;
        
    // If enabled, splash damage to players/other enemies when the enemy dies/explodes will use a linear falloff calculation. This is negated if "Use Fixed Amount" is enabled.
    UPROPERTY(EditAnywhere, BlueprintReadWrite, SaveGame)
    bool UseLinearFalloffAmount;

    // The maximum amount of damage to players/other enemies when the enemy dies/explodes for the linear falloff calculation.
    UPROPERTY(EditAnywhere, BlueprintReadWrite, SaveGame)
    FAscMapKitDifficultyFloatStruct LinearFalloffAmount;

    // If enabled, splash damage to players/other enemies when the enemy dies/explodes will use a fixed damage amount. This overrides "Use Linear Falloff Amount".
    UPROPERTY(EditAnywhere, BlueprintReadWrite, SaveGame)
    bool UseFixedAmount;

    // The amount of damage to players/other enemies when the enemy dies/explodes for the fixed amount.
    UPROPERTY(EditAnywhere, BlueprintReadWrite, SaveGame)
    FAscMapKitDifficultyFloatStruct FixedAmount;

    // The radius used to apply the splash damage to players/other enemies when the enemy dies/explodes.
    // * 2000 is the size of a cube.
    UPROPERTY(EditAnywhere, BlueprintReadWrite, SaveGame, meta = (ClampMin = "0.0", ClampMax = "999999.9999"))
    float ImpulseRadius;

    // How much impulse is applied to players during the splash damage effect when the enemy dies/explodes.
    UPROPERTY(EditAnywhere, BlueprintReadWrite, SaveGame, meta = (ClampMin = "0.0", ClampMax = "999999.9999"))
